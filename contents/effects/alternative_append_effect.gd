	...
	for existing_item in effects:
		if existing_item[0] == key_hash:
			if not applied[0]:
				...
				applied[0] = true
			key_index = 0
		if existing_item[0] == key2_hash:
			if not applied[1]:
				existing_item[1] += value2
				applied[1] = true
			key_index = 1
	if not applied[0]:
		effects.append([key_hash, value])
		key_index = 0
	if not applied[1]:
		effects.append([key2_hash, value2])
		key_index = 1

	# 这两个在upapply的时候不会擦除
	effects.append([key_hash if key_index else key2_hash, 0]) #最后一个元素是key，先append key2；反之亦然
	effects.append([key2_hash if key_index else key_hash, 0]) #append另一个
...
func unapply(player_index: int) -> void:
	...
	for i in effect_items.size():
		...
		if not applied[0] and effect_item[0] == key_hash:
			effect_item[1] -= value
			if effect_item[1] == 0:
				item_to_remove.push_front(i)
			applied[0] = true
		elif not applied[1] and effect_item[0] == key2_hash:
