...
var to_stat_hash = Keys.empty_hash
...
func duplicate(subresources := false) -> Resource:
	...
	duplication.to_stat_hash = self.to_stat_hash
	return duplication
...
func apply(player_index: int) -> void:
	...
	effects[custom_key_hash].push_back(to_array())


func unapply(player_index: int) -> void:
	var effects = RunData.get_player_effects(player_index)
	effects[custom_key_hash].erase(to_array())


func to_array() -> Array:
	return [pct_converted, key_hash, value, offset, keep_value, to_stat_hash, to_stat_scaling, storage_method, is_negative_key]
