...
# item_brolab_佛手_422, item_brolab_面具_422
var debug_item_name: String= ""

var weapon_to_get: WeaponData = null
var item_to_get: ItemData = null # for special item effect (duplicate_item, increase_tier_on_reroll)

var weapon_id: String = ""
var extra_item_id: String = ""
...
func apply(player_index: int) -> void:
	if weapon_id.empty():
		weapon_to_get = _get_rand_weapon(player_index)
	RunData.add_weapon(weapon_to_get, player_index)
	if item_to_get != null:
		RunData.add_item(item_to_get, player_index)
		item_to_get = null
...
func get_args(player_index: int) -> Array:
	...
	return [weapon_id, extra_item_id]
...
func _get_rand_weapon(player_index: int) -> WeaponData:
	...
	if !debug_item_name.empty():
		if debug_item_name.begins_with("weapon_"):
			item_for_effect = ItemService.get_element(ItemService.weapons, debug_item_name)
			debug_item_name = ""
		else:
			item_for_effect = ItemService.get_element(ItemService.items, debug_item_name)
			debug_item_name = ""
	else:
	...
	weapon_id = "%s%s" % [tr(weapon.name), ItemService.get_tier_number(weapon.tier)]
	if weapon.is_cursed:
		weapon_id += "([color=#%s]%s[/color])" % [Utils.CURSE_COLOR.to_html(), tr("BROLAB_CURSED_TEXT")]
	
	if item_for_effect is WeaponData:
		extra_item_id = "%s%s" % [tr(item_for_effect.name), ItemService.get_tier_number(item_for_effect.tier)]
	else:
		extra_item_id = tr(item_for_effect.name)

	if item_for_effect.is_cursed:
		extra_item_id += "([color=#%s]%s[/color])" % [Utils.CURSE_COLOR.to_html(), tr("BROLAB_CURSED_TEXT")]
