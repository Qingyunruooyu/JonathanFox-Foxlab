...
func _get_rand_item_for_wave(wave: int, player_index: int, type: int, args: GetRandItemForWaveArgs):
	if wave < 13 and type == TierData.ITEMS:
		...
		if RunData.get_nb_item(Utils.character_foxlab_loong_rider_hash, player_index) > 0:
			for item in foxlab_banned_items_earlier:
				if item[0].my_id_hash == Keys.generate_hash("item_foxlab_spacetime_anchor"):
					continue
				args.excluded_items.append(item)
		else:
